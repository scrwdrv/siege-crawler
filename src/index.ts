...
(function exec() {
    ...
    else if (!paused) setTimeout(exec, options.rate);
})();
...
function printResult() {
    ...
    function format(history: { url: string; status: string; responseTime: number }) {
...
function siege(uri: string) {
    ...
    get(uri, (err, body) => {
        ...
        if (result.time[0] < dt) result.time[0] = dt;
        if (result.time[1] > dt) result.time[1] = dt;
        result.time[2] += dt;
        ...
        $('a').each((i, elem) => {
            ...
            if (!href) return;
            if (regs.chinese.test(href)) href = encodeURI(href);
        ...
        function fixUrl(str: string, specificBase?: URL) {
